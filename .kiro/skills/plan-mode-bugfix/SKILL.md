...
Create `.kiro/specs/{bug-name}/bugfix.md` with four required sections:
...
Create `.kiro/specs/{bug-name}/design.md` with four required sections:
...
Create `.kiro/specs/{bug-name}/tasks.md`:
...
```
.kiro/specs/{component}-{symptom}-fix/
├── .config.kiro    # specType: "bugfix"
├── bugfix.md       # Replaces requirements.md
├── design.md
└── tasks.md
```
...
Examples:
- `checkout-zero-quantity-crash-fix`
- `login-session-timeout-fix`
- `api-null-response-fix`
