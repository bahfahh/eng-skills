...
| `frontend-vite-react-checklist` | Vite + React 專屬開發品質清單（兩種模式）：(1) coding/plan 模式—補充 Vite config、React Router v6+、TanStack Query、env var 安全等 todolist；(2) code review 模式—審查 routing、state 管理策略、bundle 優化、build 設定。涵蓋 AI 常遺漏的 Vite+React 專屬陷阱（alias drift、barrel import、lazy route 缺失等） |
