...
- For performance phases (4–6), load the `react-official-best-practices` skill if available
- For Next.js file conventions, RSC boundaries, async APIs, load the `next-official-best-practices` skill if available
...
- Load the `next-official-upgrade` skill and follow its instructions
...
| Performance rules | Load `react-official-best-practices` skill |
| Next.js conventions / RSC / async APIs | Load `next-official-best-practices` skill |
